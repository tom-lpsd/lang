#ifndef _TYPELIST_HPP
#define _TYPELIST_HPP
...
{
  const static int value = 1 + Length<U>::value;
};
...
{
  const static int temp = IndexOf<Tail,T>::value;
  const static int value = (temp == -1) ? -1 : 1+temp;
};
...
{
  typedef typename Erase<typename NoDuplicates<Tail>::Result, Head>::Result L;
  typedef Typelist<Head, L> Result;
};
...
template<class Head, class Tail, class U>
...
{
  typedef Typelist<U, typename ReplaceAll<Tail, Head, U>::Result> Result;
};
